...
const howManyTimes = (words, word) => words.filter((w) => w === word).length;

// Iteration 2 | Number Sequence
const createSequence = (number) => {
  if (number === 0) {
    return [];
  } else {
    let result = [];
...
const multiplyBy = (nums, multiplier) => {
  const result = [];
  nums.forEach((num) => result.push(num * multiplier));
  return result;
};
